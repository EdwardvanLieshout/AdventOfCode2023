...
const letFall = (bricks: Brick[]): void => {
    for (let brick of bricks) {
        ...
        const bricksBelow = bricks
            .filter((b) => b.z1 === brick.z0 - 1)
            .filter((b) => intersectsX(brick.x0, brick.x1, b.x0, b.x1) && intersectsY(brick.y0, brick.y1, b.y0, b.y1));
...
console.log(bricksList.sort((a, b) => b.z1 - a.z1));
let ans = 0;
for (let brick of bricksList) {
    ...
    if (tempBricksList === JSON.stringify(testBricksList)) {
        ans++;
    }
}
